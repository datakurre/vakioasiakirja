...
| Paragraph separation | block style (default, follows 6.4.3): a ≈ 11,6 pt gap (standard requires ≥ 10 pt), no first-line indent; compact `nogap` style: no gap, a one-column first-line indent on a paragraph following another |
...
| 5.1–5.2 Metatiedot | Visible metadata and stored document properties must agree; minimum metadata includes the laatija (author); basic metadata area indented 9,2 cm from the left margin; doctype bold first, then date, document ID and confidentiality; two paragraph gaps before subject/title when there is no extra metadata | The metadata block is built from the same commands that fill the PDF document properties (title, subject, author, keywords, creation/modification dates and `fi` language via hyperref); `\author` records the laatija; a `\date` in the d.m.yyyy format becomes both the PDF creation and modification date (`\modified` overrides the latter); 9,2 cm indent and the line order/spacing rules are hard-coded in `\maketitle` and the header |
| 5.2.1 Aihe ja pääotsikko | Subject bold; main title bold and 2–4 pt larger than body text; placed adjacent before the body | `\subject` renders bold at body size; `\title` renders bold at the body size + 3 pt (at any base size), directly below the subject at the left margin, separated by the title's own line spacing as in the standard's model documents |
...
| 6.1 Logo | Logo (or organization/person name as text) in the top-left corner; layout must adapt to the logo's size | `\logo{}` accepts any box — an `\includegraphics` image or styled text; the page header height grows automatically to fit a tall logo, shrinking the text area accordingly, but a logo taller than `\logomaxheight` (20 mm by default) is scaled down proportionally first |
...
| 6.4.3 Kappaleväli | Paragraphs separated by paragraph spacing of at least 10 pt, not by blank lines; no first-line indent; page breaks by feature, not empty lines | The default block style follows this literally — a 0,88-line ≈ 11,6 pt gap (`\sfsparskip`) between paragraphs, no first-line indent; the optional `nogap` style instead runs paragraphs together and sets a paragraph that follows another apart with a one-column first-line indent. Use `\clearpage` for manual breaks |
| 6.5.1–6.5.2 Taulukot, kuvat ja kaaviot | Real (not image) tables with the top row as a header row, caption above the table; figures in the text flow at the text indent with the caption in their immediate proximity | Captions via the *caption* package: ragged, never centered, label separated by a quad, table captions configured above with one paragraph gap before the table and the caption kept attached to it; `\captionof{table}`/`\captionof{figure}` for in-flow material |
| 6.5.3 Luettelot | Lists made with the list tool, indented, separated from text by at least one paragraph gap, at least one space after the marker | Preconfigured *enumitem* lists: items marked with the en dash of Finnish convention; the first level starts at the body indent (no extra margin) and is set off from the surrounding text by one paragraph gap in the block style (flush in the compact `nogap` style); nested levels indent further and sit flush against their parent, adding no extra margin; spacing overridable per list |
...
| 6.6.1 Omakätinen allekirjoitus | Reserve 3–5 paragraph gaps for a handwritten signature; name + comma + role below; multiple signers stacked | `\handsignature{Name, role}` reserves three paragraph gaps and prints its line verbatim; repeat to stack |
| 6.6.2 Sähköinen allekirjoitus | The sentence "Tämä asiakirja on sähköisesti allekirjoitettu." followed by each signer's name, role and email | `esignatures` environment emits the sentence (`\esignaturestext`); one `\esignee{Name, role}{email}` per signer |
| 6.7 Liitteet, jakelu ja tiedoksi | After the signatures, in this order, each label with its items one per line, groups separated by paragraph gaps | `\attachments`, `\distribution`, `\forinformation` render the *Liitteet* / *Jakelu* / *Tiedoksi* labels (`\attachmentsname` etc.) at the margin with the listed items at the body indent; the first adds the separating paragraph gap automatically |
| 6.8 Organisaation yhteystiedot | Contact details at the end of the document in the body area (not only in a footer), organization name first | `\contactinfo{Name}{lines}` + `\makecontactinfo` at the document end, an extra paragraph gap above it; name in bold, lines without paragraph gaps |
