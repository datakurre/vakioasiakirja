...
| 6.5.3 Luettelot | Lists made with the list tool, indented, separated from text by at least one paragraph gap, at least one space after the marker | Preconfigured *enumitem* lists: one paragraph gap above/below, items marked with the en dash of Finnish convention; the first level starts at the body indent (no extra margin), nested levels indent further; spacing overridable per list |
