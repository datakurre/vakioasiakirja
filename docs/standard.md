...
| Paragraph separation | run-in layout (default): no gap, a one-column first-line indent on a paragraph following another; block layout (`no-runin`): a ≈ 11,6 pt gap (standard requires ≥ 10 pt), no indent |
...
| 6.4.3 Kappaleväli | Paragraphs separated by paragraph spacing of at least 10 pt, not by blank lines; no first-line indent; page breaks by feature, not empty lines | The `no-runin` block layout follows this literally — a 0,88-line ≈ 11,6 pt gap (`\sfsparskip`) between paragraphs, no first-line indent; the default run-in layout instead runs paragraphs together and sets a paragraph that follows another apart with a one-column first-line indent. Use `\clearpage` for manual breaks |
...
| 6.5.3 Luettelot | Lists made with the list tool, indented, separated from text by at least one paragraph gap, at least one space after the marker | Preconfigured *enumitem* lists: items marked with the en dash of Finnish convention; the first level starts at the body indent (no extra margin) and is set off from the surrounding text by the paragraph spacing in force (a paragraph gap in the block layout, nothing in run-in); nested levels indent further and sit flush against their parent, adding no extra margin; spacing overridable per list |
