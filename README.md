...
| `agenda` | Meeting-agenda heading style: regular weight, numbered with a trailing period `1. Otsikko` (an alternative the standard explicitly permits). Starred headings such as `\section*{Esityslista}` stay bold. |
...
Set these before `\begin{document}`. Only `\doctype`, `\date` and `\title`
are usually needed; everything else is optional and omitted from the
output when unset.
...
| `\title{Verkkosivujen uudistaminen}` | Pääotsikko | Bold, 2–4 pt larger than body text |
...
The metadata is also written into the PDF document properties
(title, subject, author) for accessibility and document management.
...
automatically as a header on every following page.
...
| Paragraph gap | 12 pt (standard requires ≥ 10 pt) |
...
| 5.1–5.2 Metatiedot | Visible metadata and stored document properties must agree; basic metadata area indented 9,2 cm from the left margin; doctype bold first, then date, document ID and confidentiality; two paragraph gaps before subject/title when there is no extra metadata | The metadata block is built from the same commands that fill the PDF document properties (title, subject, author via hyperref); 9,2 cm indent and the line order/spacing rules are hard-coded in `\maketitle` and the header |
| 5.2.1 Aihe ja pääotsikko | Subject bold; main title bold and 2–4 pt larger than body text; placed adjacent before the body | `\subject` renders bold at body size; `\title` renders bold at `\Large` (+3,4 pt at the 11 pt base), directly below the subject at the left margin |
...
| 6.4 Otsikointi ja jäsentely | Headings hang at the left margin and must stand out from body text; at most three levels recommended; running numbering from 1, no trailing period — though an established trailing-period style is also permitted | `\section`, `\subsection`, `\subsubsection` (numbering depth 3) hang into the margin, bold, numbered `1`, `1.1`, `1.1.1`; the `agenda` option switches to the permitted alternative `1.` style in regular weight; `\section*` for unnumbered bold headings; `\marginlabel` for unnumbered regular-weight labels |
| 6.4.2 Leipäteksti | Left-aligned (ragged right) single column; body indented 2,3 cm from the left margin; line width at most 15,7 cm; font size 11–12 pt; line spacing typically 1,1–1,2 | Text width is exactly 15,7 cm at the 2,3 cm indent; ragged right via *ragged2e* (hyphenation kept); 11 pt default / `12pt` option; Palatino's 13,6 pt leading gives 1,24 spacing, just above the typical band, chosen for readability |
| 6.4.3 Kappaleväli | Paragraphs separated by paragraph spacing of at least 10 pt, not by blank lines; no first-line indent; page breaks by feature, not empty lines | Paragraph gap is 12 pt of glue (`\parskip`), first-line indent 0; use `\clearpage` for manual breaks |
